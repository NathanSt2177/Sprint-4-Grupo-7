    ...
    try:
        ...
        if request.method == 'GET': 
            ...
            if attempt is not None:
                db.execute(
                    'INSERT INTO activationlink (state, id) VALUES (?,?)', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                db.execute(
                    'INSERT INTO forgotlink (state, userid) VALUES (?,?)', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'INSERT INTO user (password, salt, id)', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
